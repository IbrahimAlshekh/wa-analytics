...
export default function InsightsPanel({ entries }) {
    ...
    const statCards = [
        ...
        streak != null && { label: streak.online ? "Online streak" : "Offline for", value: streak.online ? `${streak.days}d` : formatDuration(streak.seconds) },
        avgOnlineSec != null && { label: "Daily avg online", value: formatDuration(avgOnlineSec) + (trendPct != null ? ` ${trendPct > 0 ? "▲" : "▼"}${Math.abs(trendPct)}%` : "") },
    ].filter(Boolean);
    return (_jsxs("div", { className: "col", style: { gap: 16 }, children: [
        statCards.length > 0 && (_jsx("div", { className: "stats", children: statCards.map((c) => (_jsxs("div", { className: "stat-card", children: [_jsx("div", { className: "label", children: c.label }), _jsx("div", { className: "value", children: c.value })] }, c.label))) })),
        patternSummary && (_jsx("div", { className: "card", children: _jsxs("span", { className: "muted", style: { fontSize: 13 }, children: ["Usually most active between ", _jsx("strong", { style: { color: "var(--accent)" }, children: patternSummary })] }) })),
        _jsxs("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }, children: [
            _jsxs("div", { className: "card", children: [_jsx("h3", { style: { marginTop: 0 }, children: "Peak Activity Hours" }), _jsx("div", { className: "muted", style: { marginBottom: 8 }, children: "Online minutes by hour of day" }), _jsx("div", { style: { width: "100%", height: 200 }, children: _jsx(ResponsiveContainer, { children: _jsxs(BarChart, { data: hourlyData, children: [_jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "rgba(127,127,127,0.2)" }), _jsx(XAxis, { dataKey: "hour" }), _jsx(YAxis, {}), _jsx(Tooltip, {}), _jsx(Bar, { dataKey: "minutes", fill: "var(--accent)" })] }) }) })] }),
            _jsxs("div", { className: "card", children: [_jsx("h3", { style: { marginTop: 0 }, children: "Most Active Days" }), _jsx("div", { className: "muted", style: { marginBottom: 8 }, children: "Online minutes by day of week" }), _jsx("div", { style: { width: "100%", height: 200 }, children: _jsx(ResponsiveContainer, { children: _jsxs(BarChart, { data: weekdayData, children: [_jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "rgba(127,127,127,0.2)" }), _jsx(XAxis, { dataKey: "day" }), _jsx(YAxis, {}), _jsx(Tooltip, {}), _jsx(Bar, { dataKey: "minutes", fill: "var(--accent)" })] }) }) })] })
        ] }),
        msgFreqData.length > 0 && (_jsxs("div", { className: "card", children: [_jsx("h3", { style: { marginTop: 0 }, children: "Message Frequency" }), _jsx("div", { className: "muted", style: { marginBottom: 8 }, children: "Messages per day (sent vs received)" }), _jsx("div", { style: { width: "100%", height: 200 }, children: _jsx(ResponsiveContainer, { children: _jsxs(BarChart, { data: msgFreqData, children: [_jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "rgba(127,127,127,0.2)" }), _jsx(XAxis, { dataKey: "date" }), _jsx(YAxis, {}), _jsx(Tooltip, {}), _jsx(Bar, { dataKey: "sent", fill: "var(--accent)", name: "Sent" }), _jsx(Bar, { dataKey: "received", fill: "var(--offline)", name: "Received" })] }) }) })] })),
        _jsxs("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }, children: [
            _jsxs("div", { className: "card", children: [_jsx("h3", { style: { marginTop: 0 }, children: "About History" }), aboutHistory.length === 0 ? (_jsx("div", { className: "muted", children: "No about changes recorded." })) : (_jsx("div", { className: "col", style: { gap: 8, maxHeight: 220, overflowY: "auto" }, children: aboutHistory.map((e, i) => (_jsxs("div", { style: { borderBottom: "1px solid var(--border)", paddingBottom: 6 }, children: [_jsx("div", { className: "muted", style: { fontSize: 11, marginBottom: 2 }, children: formatDatetime(e.at) }), _jsx("div", { style: { fontSize: 13 }, children: e.text || _jsx("em", { className: "muted", children: "(empty)" }) })] }, i))) }))] }),
            _jsxs("div", { className: "card", children: [_jsx("h3", { style: { marginTop: 0 }, children: "Profile Picture History" }), pictureHistory.length === 0 ? (_jsx("div", { className: "muted", children: "No profile pictures recorded." })) : (_jsx("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, maxHeight: 220, overflowY: "auto" }, children: pictureHistory.map((e, i) => (_jsxs("a", { href: e.url, target: "_blank", rel: "noreferrer", style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }, children: [_jsx("img", { src: e.url, alt: formatDatetime(e.at), style: { width: 60, height: 60, objectFit: "cover", borderRadius: 8, border: "1px solid var(--border)" } }), _jsx("span", { className: "muted", style: { fontSize: 10 }, children: formatDate(e.at) })] }, i))) }))] })
        ] })
    ] }));
}
...
function computeAvgSessionDuration(entries) {
    ...
    for (const p of presence) {
        if (p.state === "available") { onlineAt = p.at; }
        else if (p.state === "unavailable" && onlineAt != null) { durations.push(p.at - onlineAt); onlineAt = null; }
    }
...
function computeMessageFrequency(entries) {
    ...
    return Object.entries(byDate).sort(([a], [b]) => a.localeCompare(b)).map(([date, v]) => ({ date: date.slice(5), ...v }));
}
...
function computeOnlinePatternSummary(hourlyData) {
    ...
    const activeHours = hourlyData.map((d, i) => ({ i, minutes: d.minutes })).filter((d) => d.minutes >= threshold).map((d) => d.i);
    if (activeHours.length === 0) return null;
    const fmt = (h) => { const suffix = h < 12 ? "am" : "pm"; const display = h % 12 === 0 ? 12 : h % 12; return `${display}${suffix}`; };
...
function computeStreak(entries) {
    ...
    if (last.state === "unavailable") return { online: false, days: 0, seconds: nowSec - last.at };
    const activeDays = new Set();
    let onlineAt = null;
    for (const p of presence) {
        if (p.state === "available") { onlineAt = p.at; }
        else if (p.state === "unavailable" && onlineAt != null) { activeDays.add(new Date(onlineAt * 1000).toISOString().slice(0, 10)); onlineAt = null; }
    }
    let streak = 0;
...
function computeDailyAvgOnline(entries) {
    ...
    for (const p of presence) {
        if (p.state === "available") { onlineAt = p.at; }
        else if (p.state === "unavailable" && onlineAt != null) { addSeconds(onlineAt, p.at); onlineAt = null; }
    }
