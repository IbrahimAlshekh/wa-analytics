...
export default function EmotionCard({
  ...
  const totalEmotionMe =
    EMOTION_KEYS.reduce((s, k) => s + (emotion.countsMe[k] ?? 0), 0) +
    emotion.laughterMsgsMe +
    emotion.questionsMe;
  const totalEmotionThem =
    EMOTION_KEYS.reduce((s, k) => s + (emotion.countsThem[k] ?? 0), 0) +
    emotion.laughterMsgsThem +
    emotion.questionsThem;
  const indexMe = totalMsgsMe > 0 ? (totalEmotionMe / totalMsgsMe) * 100 : 0;
  const indexThem =
    totalMsgsThem > 0 ? (totalEmotionThem / totalMsgsThem) * 100 : 0;
  const totalMsgs = totalMsgsMe + totalMsgsThem;
  const indexCombined =
    totalMsgs > 0 ? ((totalEmotionMe + totalEmotionThem) / totalMsgs) * 100 : 0;
      ...
      <CardContent className="pt-0 flex flex-col gap-4">
        <div className="rounded-md bg-muted/40 p-3 flex flex-col gap-2">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
            {t("analytics.emotion.indexTitle")}
          </p>
          <div className="grid grid-cols-3 gap-3">
            <StatItem
              label={t("analytics.emotion.indexYou")}
              value={`${indexMe.toFixed(1)}%`}
            />
            <StatItem
              label={t("analytics.emotion.indexThem", { name: contactName })}
              value={`${indexThem.toFixed(1)}%`}
            />
            <StatItem
              label={t("analytics.emotion.indexCombined")}
              value={`${indexCombined.toFixed(1)}%`}
            />
          </div>
        </div>
        ...
        {(hasLaughs || hasQuestions) && (
          <div className="flex flex-col gap-1.5">
            ...
            {hasLaughs && (
              <EmotionRow
                icon="😂"
                label={t("analytics.emotion.laughsLabel")}
                me={emotion.laughterMsgsMe}
                them={emotion.laughterMsgsThem}
              />
            )}
            {hasQuestions && (
              <EmotionRow
                icon="❓"
                label={t("analytics.emotion.questionsLabel")}
                me={emotion.questionsMe}
                them={emotion.questionsThem}
              />
            )}
          </div>
        )}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {EMOTION_KEYS.map((k) => {
              ...
              return (
                <EmotionRow
                  key={k}
                  icon={EMOTION_ICONS[k]}
                  label={t(`analytics.emotion.categories.${k}`)}
                  me={me}
                  them={them}
                />
              );
