...
export default function LiveStatusCard({
  ...
  const lastSeenText =
    !isOnline && lastPresence
      ? lastPresence.lastSeen
        ? t("contactDetail.lastSeen", {
            time: formatRelative(lastPresence.lastSeen),
          })
        : t("contactDetail.lastSeen", { time: formatRelative(lastPresence.at) })
      : null;

  const elapsed =
    isOnline && sessionStart != null ? formatElapsed(sessionStart) : null;
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            <span
              className={cn(
                ...
                isOnline
                  ? "bg-primary shadow-[0_0_0_4px_oklch(0.723_0.173_145/0.2)]"
            ...
            <div>
              <p className="font-semibold text-sm">
                {isOnline
                  ? t("contactDetail.online")
                  : t("contactDetail.offline")}
              </p>
              {elapsed && (
          ...
          {isOnline && sessionStart != null && (
            <span className="text-xs text-primary font-medium">
              {t("contactDetail.onlineSince", {
                time: formatTime(sessionStart),
              })}
            </span>
          )}
        </div>
