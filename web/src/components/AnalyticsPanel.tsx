...
function EmotionCard({ emotion, contactName, totalMsgsMe, totalMsgsThem }: {
...
function EmotionRow({ icon, label, me, them }: { icon: string; label: string; me: number; them: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs text-muted-foreground w-8 tabular-nums shrink-0">{formatCount(them)}</span>
    </div>
  );
}
